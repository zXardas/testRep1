public class Main {
    /*
1. Создать классы Собака, Лошадь, Птица и Кот с наследованием от класса Животное.
2. Животные могут выполнять действия: бежать, плыть, перепрыгивать препятствие. В качестве параметра каждому методу передается величина, означающая или длину препятствия (для бега и плавания), или высоту (для прыжков).
 3. У каждого животного есть ограничения на действия (бег: кот 200 м., собака 500 м., Лошадь 1500 м., Птица 5 м.,; прыжок: кот 2 м., собака 0.5 м., Лошадь 3 м., Птица 0.2 м.
 плавание: кот и птица не умеет плавать, собака 10 м., лошадь 100 м.).

4. При попытке животного выполнить одно из этих действий, оно должно сообщить результат. (Например, dog1.run(150); -> результат: 'Пёсик пробежал!')
5. * Добавить животным разброс в ограничениях. То есть у одной собаки ограничение на бег может быть 400 м., у другой 600 м.
            */

    public static void main(String[] args) {

Dog dog = new Dog("dog",0.5f,10);
Horse horse = new Horse("horse",1500,100,10);
Cat cat = new Cat("cat",200,0,2);
Bird bird = new Bird("bird",5,0,0.2f);

            System.out.println("______________dog");
dog.run(100);
dog.run(1000);
dog.swim(0.1f);
dog.swim(100);
dog.jump(0.1f);
dog.jump(100);
            System.out.println("______________horse");
horse.run(100);
horse.run(2000);
horse.swim(1);
horse.swim(100);
horse.jump(0.1f);
horse.jump(100);
            System.out.println("______________bird");
bird.run(4);
bird.run(1000);
bird.swim(1);
bird.swim(100);
bird.jump(0.1f);
bird.jump(100);
            System.out.println("______________cat");
cat.run(100);
cat.run(1000);
cat.swim(1);
cat.swim(100);
cat.jump(0.1f);
cat.jump(100);
    }
}
