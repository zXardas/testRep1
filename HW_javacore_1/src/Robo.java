...
public class Robo implements Movin {
    private String name;
    ...
    public Robo(String name) {
        ...
        System.out.println("Robo enduranceRun " + this.enduranceRun);
        System.out.println("Robo enduranceJump " + this.enduranceJump +"\n"
        );
    ...
    public void run(RunDistance r) {
        if (cannMove) {
            if (this.enduranceRun - r.getRunDistance() < 0) {
                cannMove = false;
                System.out.printf("Robo %s tired and cant move! %sm too difficult distance fo run at this momnt\n", name, r.getRunDistance());
            } else {
                this.enduranceRun -= r.getRunDistance();
                System.out.printf("Robo %s run %sm! And can run %sm\n", name, r.getRunDistance(), enduranceRun);
            }
        }
    }
    public void jump(JumpDistance j) {
        if (cannMove) {
            if (this.enduranceJump - j.getJumpDistance() < 0) {
                cannMove = false;
                System.out.printf("Robo %s tired and cant move! %sm too difficult jump distance\n", name, j.getJumpDistance());
            } else {
                this.enduranceJump -= j.getJumpDistance();
                System.out.printf("Robo %s jump %sm! And can jump %sm\n", name, j.getJumpDistance(), enduranceJump);
            }
        }
    }
}